...
export function TeamsPage() {
  ...
  return (
    <section className="page-content">
      ...
      <div className="card-list">{teamsQuery.data?.items.map((team) => <Link className="team-card team-card-link" key={team.id} to={`/teams/${team.id}`}><div className="team-badge">{team.name.slice(0, 2).toUpperCase()}</div><div><h2>{team.name}</h2><p>{team.category} · {team.gender === 'unspecified' ? 'Sin género indicado' : team.gender}</p></div><span className="team-status">{team.status === 'active' ? 'Activo' : team.status}</span><span className="row-chevron">›</span></Link>)}</div>
    </section>
  )
}
