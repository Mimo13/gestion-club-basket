...
type MatchPhaseValue = 'preliminary' | 'regular_league' | 'playoffs' | 'cup' | 'friendly' | 'tournament' | 'other'
type LeagueTierValue = 'bronze' | 'silver' | 'gold' | ''
type ActivityForm = { type: 'training' | 'match'; teamId: string; startsAt: string; endsAt: string; venueName: string; notes: string; opponentName: string; isHome: boolean; competition: string; phase: MatchPhaseValue; leagueTier: LeagueTierValue }
...
function emptyForm(teamId = ''): ActivityForm {
  ...
  return { type: 'training', teamId, startsAt: dateTimeInputValue(start.toISOString()), endsAt: dateTimeInputValue(end.toISOString()), venueName: '', notes: '', opponentName: '', isHome: true, competition: '', phase: 'regular_league', leagueTier: '' }
}

function formFromActivity(activity: Activity): ActivityForm {
  return { type: activity.type, teamId: activity.teamId, startsAt: dateTimeInputValue(activity.startsAt), endsAt: activity.endsAt ? dateTimeInputValue(activity.endsAt) : '', venueName: activity.venueName ?? '', notes: activity.notes ?? '', opponentName: activity.opponentName ?? '', isHome: activity.isHome ?? true, competition: activity.competition ?? '', phase: activity.matchPhase ?? 'regular_league', leagueTier: activity.leagueTier ?? '' }
}
...
function toInput(form: ActivityForm): CreateActivityInput {
  ...
  return form.type === 'match' ? { ...base, type: 'match', opponentName: form.opponentName, isHome: form.isHome, competition: form.competition || null, phase: form.phase, leagueTier: form.leagueTier || null } : { ...base, type: 'training' }
}
